...
static unsigned exam_powerlaws2()
{
    ...
    ex e2 = e1.subs(a==3);
    if(e2.is_equal(pow(3,b)*pow(x,b))) return 0;
    else /*if (!(is_exactly_a<power>(e2) &&
...
static unsigned exam_powerlaws6()
{
    ...
    ex e1 = pow(5*pow(3*a*b*x*y*p*q,2),7*half*c).power_expand();
    ex e2 = pow(q,(7*c))*pow(45,(7/ex(2)*c))*pow(pow(x,2),(7/ex(2)*c))*pow(pow(b,2)*pow(a,2),(7/ex(2)*c))*pow(p,(7*c))*pow(pow(y,2),(7/ex(2)*c));
    if (!e1.is_equal(e2)) {
        clog << "Could not expand exponents with positive bases in " << (e1) << endl;
        return 1;
    }

    ex e3 = pow(-pow(-a*x*p,3)*pow(b*y*p,3),half*c).power_expand().normal();
