...
{
    using ginacsym::asin; using ginacsym::acos;
    using ginacsym::asinh; using ginacsym::acosh; using ginacsym::atanh;
...
static unsigned inifcns_consist_log()
{
    ...
    ex e2 = log(7)+log(p)+log(pow(q,3))+log(-z*a*w)+log(pow(b,2));
    if (!e1.is_equal(e2))
        ++result;
