 ...
 *  working of that other package from the user of ginacsym.  It must either 
...
/** ctor from C-style string.  It also accepts complex numbers in ginacsym
 *  notation like "2+5*I". */
numeric::numeric(const char *s)
{
	...
	do {
		...
		if (term.find('.')!=std::string::npos || term.find('E')!=std::string::npos) {
			// CLN's short type cl_SF is not very useful within the ginacsym
 ...
 *  and instead of 3.99168L7 it prints 3.99168E7.  This is fine in ginacsym as
 ...
 *  This method may be used when mixing ginacsym and CLN in one project.
 */
 ...
 *
 *  @param x complex number
 *  @param b complex number, base
 *  @return  arbitrary precision numerical log(x,b).
 *  @exception pole_error("log(): logarithmic pole",0) */
const numeric log(const numeric &x,const numeric &b)
{
    if (x.is_zero() || b.is_zero())
        throw pole_error("log(): logarithmic pole",0);
    return numeric(cln::log(x.to_cl_N(),b.to_cl_N()));
}
