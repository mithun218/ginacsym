...
ex expairseq::map(map_function &f) const
{
	...
	for (auto & it : seq)
        v.push_back(split_ex_to_pair(f.expr_visitor(recombine_pair_to_ex(it))));

	if (overall_coeff.is_equal(default_overall_coeff()))
		return thisexpairseq(std::move(v), default_overall_coeff(), true);
	else {
        ex newcoeff = f.expr_visitor(overall_coeff);
...
void expairseq::make_flat(const exvector &v)
{
	...
	for (auto & cit : v) {
		...
		if (is_a<mul>(*this) && (!do_idx_rename) &&
            ...
            do_idx_rename = false; //I have done true -> false (change by me)
	}
...
void expairseq::make_flat(const epvector &v, bool do_index_renaming)
{
	...
	for (auto & cit : v) {
		...
		if ((!really_need_rename_inds) && is_a<mul>(*this) &&
            ...
            really_need_rename_inds = false;//I have done true -> false (change by me)
	}
...
void expairseq::combine_same_terms_sorted_seq()
{
	...
	while (itin2!=last) {
        if (itin1->rest.compare(itin2->rest)==0 and likely(not is_exactly_a<infinity>(itin1->rest))) {
