/** @file indexed.cpp
 *
 *  Implementation of ginacsym's indexed expressions. */
...
void indexed::read_archive(const archive_node &n, lst &sym_lst)
{
	...
	if (!n.find_ex("symmetry", symtree, sym_lst)) {
		// ginacsym versions <= 0.9.0 had an unsigned "symmetry" property
		unsigned symm = 0;
...
ex ex::simplify_indexed(unsigned options) const
{
	exvector free_indices, dummy_indices;
	scalar_products sp;
	return ginacsym::simplify_indexed(*this, free_indices, dummy_indices, sp);
}
...
{
	return ginacsym::symmetrize(*this, get_free_indices());
}

/** Antisymmetrize expression over its free indices. */
ex ex::antisymmetrize() const
{
	return ginacsym::antisymmetrize(*this, get_free_indices());
}

/** Symmetrize expression by cyclic permutation over its free indices. */
ex ex::symmetrize_cyclic() const
{
	return ginacsym::symmetrize_cyclic(*this, get_free_indices());
}
