...
/** Return pointer to first symbol found in expression.  Due to ginacsym's
...
ex quo(const ex &a, const ex &b, const ex &x, bool check_args)
{
    ...
    //    // determine all symbols in a,b
    //    find_symbols_map findsymbolsa,findsymbolsb;
    //    findsymbolsa.expr_visitor(a);
    //    findsymbolsb.expr_visitor(b);

    //    lst syms;
    //    std::set<std::string> symsstr;
    //    std::string str;
    //    size_t symstrmaxsize=0;
    //    for (auto & i : findsymbolsa.syms ) {
    //        syms.append(i);
    //        str=to_string(i);
    //        symsstr.insert(str);
    //        if(symstrmaxsize<str.size()) symstrmaxsize=str.size();
    //    }
    //    for (auto & i : findsymbolsb.syms ) {
    //        syms.append(i);
    //        str=to_string(i);
    //        symsstr.insert(str);
    //        if(symstrmaxsize<str.size()) symstrmaxsize=str.size();
    ...
    //    fmpq_mpoly_ctx_init(mctx, symsstr.size(), ORD_LEX);
    //    fmpq_mpoly_init(fa, mctx);
    //    fmpq_mpoly_init(fb, mctx);
    //    fmpq_mpoly_init(fq, mctx);
    //    //flint_set_num_threads(3);
    //    fmpq_mpoly_set_str_pretty(fa,to_string(a).c_str(), vars, mctx);
    //    fmpq_mpoly_set_str_pretty(fb,to_string(b).c_str(), vars, mctx);

    //    fmpq_mpoly_div(fq, fa, fb, mctx);
    //    std::string qstr=fmpq_mpoly_get_str_pretty(fq, vars, mctx);

    //    fmpq_mpoly_clear(fa, mctx);
    //    fmpq_mpoly_clear(fb, mctx);
    //    fmpq_mpoly_clear(fq, mctx);
    //    fmpq_mpoly_ctx_clear(mctx);
    //    flint_cleanup_master();

    //    return generator.exGeneratorFromString(qstr);
    ...
    if (a.is_equal(b))
        return _ex1;
#endif
    if (check_args && (!a.is_polynomial(x) || !b.is_polynomial(x)))
        throw(std::invalid_argument("quo: arguments must be polynomials over the variable "+to_string(x)));

    //(by me)
    //(!a.info(info_flags::rational_polynomial) || !b.info(info_flags::rational_polynomial))
    ///Making leadding coefficient of b (divident), 1.
    ///Making all coefficient of b (divident) rational with variable. Then,
    ///quotient becomes polynomial over all variables.
    ///Ex: i) x^4 by c x^2 + b x + a, x^4/c*(x^2+b/c*x+a/c) -> (x^2+quosym_1*x+quosym_0)/c, where quosym_1=b/c,quosym_0=a/c.
    /// ii) x^4 by x^2 + 1/sqrt(3) x + a, x^2+b*x+a -> x^2+quosym_1*x+a, where quosym_1=1/sqrt(3).
    ex r = a.expand(), B=b.expand();
    if(r.degree(x)>=B.degree(x)){
        nonRatsubs nonRatsubsr(x);
        if(!r.info(info_flags::rational_polynomial)){
            r=nonRatsubsr.expr_visitor(r);
        }
        nonRatsubs nonRatsubsb(x);
        if(!B.info(info_flags::rational_polynomial)){
            B=nonRatsubsr.expr_visitor(B);
        }

        exmap symmap;
        ex b_lcoeff=B.lcoeff(x);
        if (!b_lcoeff.is_equal(_ex1) && !b_lcoeff.is_equal(B)) {// if x is not present in B lcoeff returns B itself,
            ex temb=pow(x,degree(B,x)),temlcoeff=r.lcoeff(x),newSym; // so checks the condition !b_lcoeff.is_equal(B)
            unsigned symnum=0;
            for (int i = 0; i < degree(B,x); ++i) {
                if(!B.coeff(x,i).is_zero()){
                    newSym=generator.symGenerator("quoCoeffSymb_" + to_string(symnum));
                    symmap[newSym]=B.coeff(x,i)/b_lcoeff;
                    temb=temb+newSym*pow(x,i);
        ...
        ex temv=dynallocate<add>(v);
        if (!nonRatsubsr.syms.empty()) {
            temv=temv.subs(nonRatsubsr.syms);
        }
        if (!nonRatsubsb.syms.empty()) {
            temv=temv.subs(nonRatsubsb.syms);
        }
        if (!symmap.empty()) {
            temv=temv.subs(symmap);
        }
        if(!b_lcoeff.is_equal(B))// if x is  present in b
            return (temv/b_lcoeff).normal();
        else return temv; // if x is not  present in b
    }
    else return _ex0;
}
...
ex rem(const ex &a, const ex &b, const ex &x, bool check_args)
{
    ...
    ///Making leadding coefficient of b (divident), 1.
    ///Making all coefficient of b (divident) rational with variable. Then,
    ///remainder becomes polynomial over all variables.
    ///Ex: i) x^4 by c x^2 + b x + a, x^4/c*(x^2+b/c*x+a/c) -> (x^2+quosym_1*x+quosym_0)/c, where quosym_1=b/c,quosym_0=a/c.
    /// ii) x^4 by x^2 + 1/sqrt(3) x + a, x^2+b*x+a -> x^2+quosym_1*x+a, where quosym_1=1/sqrt(3).
    ex r = a.expand(), B=b.expand();
    if(r.degree(x)>=B.degree(x)){
        ...
        if (!b_lcoeff.is_equal(_ex1) && !b_lcoeff.is_equal(B)) {// if x is not present in B lcoeff returns B itself,
            ex temb=pow(x,degree(B,x)),temlcoeff=r.lcoeff(x),newSym;// so checks the condition !b_lcoeff.is_equal(B)
            static unsigned symnum=0;
            for (int i = 0; i < degree(B,x); ++i) {
                if(!B.coeff(x,i).is_zero()){
                    newSym=generator.symGenerator("remCoeffSymb_" + to_string(symnum));
                    symmap[newSym]=B.coeff(x,i)/b_lcoeff;
                    temb=temb+newSym*pow(x,i);
                    symnum=symnum+1;
                }
        ...
        if (!nonRatsubsr.syms.empty()) {
            r=r.subs(nonRatsubsr.syms);
        }
        if (!nonRatsubsb.syms.empty()) {
            r=r.subs(nonRatsubsb.syms);
        }
        if (!symmap.empty()) {
            r=r.subs(symmap);
        }
...
ex add::smod(const numeric &xi) const
{
	...
	for (auto & it : seq) {
		GINAC_ASSERT(!is_exactly_a<numeric>(it.rest));
		numeric coeff = ginacsym::smod(ex_to<numeric>(it.coeff), xi);
		if (!coeff.is_zero())
			newseq.push_back(expair(it.rest, coeff));
	}
	GINAC_ASSERT(is_exactly_a<numeric>(overall_coeff));
	numeric coeff = ginacsym::smod(ex_to<numeric>(overall_coeff), xi);
...
ex mul::smod(const numeric &xi) const
{
	...
	GINAC_ASSERT(is_exactly_a<numeric>(overall_coeff));
	mulcopy.overall_coeff = ginacsym::smod(ex_to<numeric>(overall_coeff),xi);
 ...
 *  @param check_args  check whether a and b are polynomials with rational
 *         coefficients (defaults to "true")
 *  @param options  see ginacsym::gcd_options
 *  @return the GCD as a new expression */
...
ex sqrfree_parfrac(const ex & a, const symbol & x)
{
	...
	ex nd = numer_denom(a);
	ex numer = nd.op(0), denom = nd.op(1);
    //checking polynomial of numer and denom (by me).//
    if(!numer.is_polynomial(x) || !denom.is_polynomial(x)) return a;
...
/** Compute square-free partial fraction decomposition of rational function **/
ex apart(const ex & a, const symbol & x)
{
    ex nd = numer_denom(a);
    ex numer = nd.op(0), denom = nd.op(1);
    //checking polynomial of numer and denom (by me).//
    ...
    if (is_exactly_a<fail>(q))
        return q=ex(0);
//    else{
    ex rembyde_numer = rem(numer, denom, x), rembyde_denom = factor(denom);
    ex rembyde=(rembyde_numer / rembyde_denom).normal();

    if(!is_a<mul>(rembyde_denom))
        return q+rembyde.normal();
    if(is_a<mul>(rembyde_denom))
    {
        unsigned mulconstcount=0;
        for (int i = 0; i < nops(rembyde_denom); ++i) {
            if(rembyde_denom.op(i).has(x))
                mulconstcount++;
        }
        if(mulconstcount<2)
            return q+rembyde.normal();
        ...
        //sqrfree_parfrac can only decompose of rational function if repeated factor is present
        //in denominator
        if (is_a<power>(rembyde_denom)) {
            return q+sqrfree_parfrac(rembyde,x);
        }
        else if (is_a<mul>(rembyde_denom)){
            for (int i = 0; i < rembyde_denom.nops(); ++i) {
                if(is_a<power>(rembyde_denom.op(i)))
                    return q+sqrfree_parfrac(rembyde,x);
            }
        }

        //do partial fraction decomposition by solvong undetermined coefficients
        ex decomposedeq=q;
        ex undetermined_numer=ex(0);
        ex undetermined_symbol;
        ex undetermined_totaleq=-rembyde_numer;
        lst undetermined_symbols,undetermined_syseq;
        int sym_num=0;
        //creating undetermined symbols
        for (size_t i = 0; i < nops(rembyde_denom); ++i) {
            if(is_a<power>(rembyde_denom.op(i)) and is_a<add>(rembyde_denom.op(i).op(0)))
                return q+rembyde;
            for (int j = 0; j < rembyde_denom.op(i).degree(x); ++j) {
                undetermined_symbol=generator.symGenerator("undetermined_"+to_string(sym_num));
                undetermined_symbols.append(undetermined_symbol);
                undetermined_numer=undetermined_numer+pow(x,j)*undetermined_symbol;
                ++sym_num;
            }

            decomposedeq = decomposedeq +undetermined_numer/rembyde_denom.op(i);
            undetermined_totaleq=undetermined_totaleq+undetermined_numer*(rembyde_denom/rembyde_denom.op(i));
            undetermined_numer=ex(0);
        }

        //constructing undetermined equations
        undetermined_totaleq=collect(expand(undetermined_totaleq),x);
        for (int i = 0; i <= (undetermined_totaleq).degree(x); ++i){
            undetermined_syseq.append((undetermined_totaleq).coeff(x,i)==0);
        }


        //solving undetermined equations and substitutions
        ex solu= lsolve(undetermined_syseq,undetermined_symbols);
        if(!nops(solu)){
            return q+rembyde;
        }

        decomposedeq=decomposedeq.subs(solu);
        if(!is_a<add>(decomposedeq)) return decomposedeq;
        ex temdecomposedeq=_ex0;
        for (std::size_t i = 0; i < nops(decomposedeq); ++i)
            temdecomposedeq=temdecomposedeq+decomposedeq.op(i).normal();
        return temdecomposedeq;
    //}
}
 ...
 *  The first factor ginacsym denotes by something like symbol1 and will record:
 ...
 *  Denoting the third term as symbol3 ginacsym is willing to re-think exp(2*x) as
 ...
 *  altered (it solves 1). Thus ginacsym can record now
...
/** Create a symbol for replacing the expression "exp(e)" (or return a previously
 *  assigned symbol). The symbol and expression are appended to repl, and the
 *  symbol is returned.
 */
static ex replace_exp_with_symbol(const ex & e, exmap & repl)
{
    // Expression already replaced? Then return the assigned symbol
    for (auto & it : repl)
        if (it.second.is_equal(exp(e)))
            return it.first;
    // Otherwise create new symbol and add to list, taking care that the
    // replacement expression doesn't itself contain symbols from repl,
    // because subs() is not recursive
    ex es = dynallocate<symbol>();
    repl.insert(std::make_pair(es, exp(e)));
    return es;
}
...
struct normal_map_function : public map_function {
    ex expr_visitor(const ex & e) override { return normal(e); }
};
...
//by me
ex function::normal(exmap & repl, exmap & rev_lookup,  lst & modifier) const
{
    if (get_serial() == exp_SERIAL::serial
        && is_a<mul>(op(0)) && (op(0).op(op(0).nops()-1)).info(info_flags::integer)) {//it implements numer_denom(exp(-x))={1,exp(x)}
        ginacsym::power p(replace_exp_with_symbol(op(0)/(op(0).op(op(0).nops()-1)),repl_E), op(0).op(op(0).nops()-1));
        return p.normal(repl, rev_lookup, modifier);
    }
    else if (get_serial() == exp_SERIAL::serial
             && (op(0)).nops()==0) {//it implements numer_denom(exp(-x))={1,exp(x)}
        ginacsym::power p(replace_exp_with_symbol(op(0),repl_E), _ex1);
        return p.normal(repl, rev_lookup, modifier);
    }
    ...
    else return dynallocate<lst>({*this, _ex1}).hold();
}
...
ex ex::normal() const
{
    ...
    // Re-insert exp functions
    e = e.subs((repl_E),subs_options::no_pattern);
