/** @file ex.cpp
 *
 *  Implementation of ginacsym's light-weight expression handles. */
...
/** Expand power in an expression.
 *  @param options  see ginacsym::expand_options */
ex ex::power_expand(unsigned options) const
{
    if (options == 0 && (bp->flags & status_flags::power_expanded)) // The "power_expanded" flag only covers the standard options; someone might want to re-expand with different options
    {    std::cout<<"power2 "<<std::endl; return *this;}
    else
        return bp->power_expand(options);
}
...
/** Helper function for the ex-from-basic constructor. This is where ginacsym's
