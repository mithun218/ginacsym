...
std::ostream &operator<<(std::ostream &os, const archive &ar)
{
	...
	os.put('R');
	os.put('C');
    write_unsigned(os, GINACSYMLIB_ARCHIVE_VERSION);
...
std::istream &operator>>(std::istream &is, archive &ar)
{
	...
	if (c1 != 'G' || c2 != 'A' || c3 != 'R' || c4 != 'C')
		throw (std::runtime_error("not a ginacsym archive (signature not found)"));
    constexpr unsigned max_version = GINACSYMLIB_ARCHIVE_VERSION;
    constexpr unsigned min_version = GINACSYMLIB_ARCHIVE_VERSION - GINACSYMLIB_ARCHIVE_AGE;
	unsigned version = read_unsigned(is);
	if ((version > max_version) || (version < min_version))
		throw (std::runtime_error("archive version " + std::to_string(version) + " cannot be read by this ginacsym library (which supports versions " + std::to_string(min_version) + " thru " + std::to_string(max_version)));
...
/** Convert archive node to ginacsym expression. */
