	...
	if (x.info(info_flags::negative))
        ...
        if (has_pi(x)) {
                ...
                if (is_exactly_a<numeric>(ExOverPi)) {
                        ...
                        if (c.is_rational()) {
                                numeric den = c.denom();
                                numeric num = mod(c.numer(), den * *_num2_p);
                                numeric fac = *_num1_p;
                                if (num > den) {
                                        num -= den;
                                        fac = *_num_1_p;
...
{
	return cosh(ginacsym::imag_part(x))*sin(ginacsym::real_part(x));
}

static ex sin_imag_part(const ex & x)
{
	return sinh(ginacsym::imag_part(x))*cos(ginacsym::real_part(x));
}
...
REGISTER_FUNCTION(sin, evalf_func(sin_evalf).
                       eval_func(sin_eval).
                       info_func(trig_info).
...
{
	return cosh(ginacsym::imag_part(x))*cos(ginacsym::real_part(x));
}

static ex cos_imag_part(const ex & x)
{
	return -sinh(ginacsym::imag_part(x))*sin(ginacsym::real_part(x));
}
...
REGISTER_FUNCTION(cos, evalf_func(cos_evalf).
                       eval_func(cos_eval).
                       info_func(trig_info).
...
{
	ex a = ginacsym::real_part(mul(x, _ex2));
	ex b = ginacsym::imag_part(mul(x, _ex2));
...
REGISTER_FUNCTION(tan, evalf_func(tan_evalf).
                       eval_func(tan_eval).
                       info_func(trig_info).
...
REGISTER_FUNCTION(cot, evalf_func(cot_evalf).
                       eval_func(cot_eval).
                       info_func(trig_info).
...
{
	ex a = ginacsym::real_part(x);
	ex b = ginacsym::imag_part(x);
...
REGISTER_FUNCTION(sec, evalf_func(sec_evalf).
                       eval_func(sec_eval).
                       info_func(trig_info).
...
REGISTER_FUNCTION(csc, evalf_func(csc_evalf).
                       eval_func(csc_eval).
                       info_func(trig_info).
...
REGISTER_FUNCTION(acos, evalf_func(acos_evalf).
                        eval_func(acos_eval).
                        info_func(asin_info).
                      ...
                      unsigned options)
{
    ...
    if (not (I*arg_pt).info(info_flags::real))
...
REGISTER_FUNCTION(atan, evalf_func(atan_evalf).
                        eval_func(atan_eval).
                        info_func(atan_info).
...
static ex atan2_eval(const ex & y, const ex & x)
{
	if (y.is_zero()) {
		...
		// atan2(0, 0) -> undefined
        if (x.is_zero())
            throw(std::runtime_error("atan2(0, 0) -> undefined."));
	...
	if (is_exactly_a<numeric>(x)
            ...
            and (!x.info(info_flags::crational) or !y.info(info_flags::crational)))
		return atan(ex_to<numeric>(y), ex_to<numeric>(x));

	// handle infinities
	if (is_a<infinity>(x) || is_a<infinity>(y)) {
        if (is_a<infinity>(x) && ex_to<infinity>(x).info(info_flags::positive))
			throw (std::runtime_error("arctan2_eval(): arctan2(unsigned_infinity, x) encountered"));
        if (is_a<infinity>(y) && ex_to<infinity>(y).info(info_flags::positive))
			throw (std::runtime_error("arctan2_eval(): arctan2(x, unsigned_infinity) encountered"));

		if (is_a<infinity>(x) && is_a<infinity>(y)) 
            return atan2_eval(ex_to<infinity>(x).get_sign(),
                      ex_to<infinity>(y).get_sign());

		if (is_a<infinity>(x)) 
            return atan2_eval(ex_to<infinity>(x).get_sign(), 0);
		if (is_a<infinity>(y)) 
            return atan2_eval(0, ex_to<infinity>(y).get_sign());
	}
...
static ex asec_eval(const ex & x)
{
	if (is_exactly_a<numeric>(x)) {
                const numeric& num = ex_to<numeric>(x);
                if (num.is_zero())
                        throw(std::runtime_error("arcsec(0) encountered."));
                if (num.is_equal(*_num1_p))
                        return _ex0;
                if (num.is_equal(*_num_1_p))
                        return Pi;
                if (!num.info(info_flags::crational))
                        return asec_evalf(x);
	}
...
static ex acsc_eval(const ex & x)
{
	if (is_exactly_a<numeric>(x)) {
                const numeric& num = ex_to<numeric>(x);
                if (num.is_zero())
                        throw(std::runtime_error("arccsc(0) encountered."));
                if (num.is_equal(*_num1_p))
                        return Pi/_ex2;
                if (num.is_equal(*_num_1_p))
                        return -Pi/_ex2;
