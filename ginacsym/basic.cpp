/** @file basic.cpp
 *
 *  Implementation of ginacsym's ABC. */
...
struct evalf_map_function : public map_function {
    ex expr_visitor(const ex & e) override { return evalf(e); }
};
...
struct evalm_map_function : public map_function {
    ex expr_visitor(const ex & e) override { return evalm(e); }
} map_evalm;
...
struct eval_integ_map_function : public map_function {
    ex expr_visitor(const ex & e) override { return eval_integ(e); }
} map_eval_integ;
...
struct derivative_map_function : public map_function {
    ...
    ex expr_visitor(const ex & e) override { return diff(e, s); }
};
...
struct expand_map_function : public map_function {
    ...
    ex expr_visitor(const ex & e) override { return e.expand(options); }
};
...
/** Function object to be applied by basic::power_expand(). */
struct power_expand_map_function : public map_function {
    unsigned options;
    power_expand_map_function(unsigned o) : options(o) {}
    ex expr_visitor(const ex & e) override { return e.power_expand(options); }
};

/** Expand power in expression, i.e. multiply it out and return the result as a new
 *  expression. */
ex basic::power_expand(unsigned options) const
{
    if (nops() == 0)
        return (options == 0) ? setflag(status_flags::power_expanded) : *this;
    else {
        power_expand_map_function map_expand(options);
        return ex_to<basic>(map(map_expand)).setflag(options == 0 ? status_flags::power_expanded : 0);
    }
}
