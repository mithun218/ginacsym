 ...
 *
 *  Implementation of the default and builtin readers (part of ginacsym's parser).
 **/
...
{
	return ginacsym::sqrt(ev[0]);
}

static ex pow_reader(const exvector& ev)
{
	return ginacsym::pow(ev[0], ev[1]);
}

static ex power_reader(const exvector& ev)
{
	return ginacsym::power(ev[0], ev[1]);
}

static ex lst_reader(const exvector& ev)
{
	return ginacsym::lst(ev.begin(), ev.end());
}
