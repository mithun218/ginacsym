/** @file power.cpp
 *
 *  Implementation of ginacsym's symbolic exponentiation (basis^exponent). */
...
bool power::info(unsigned inf) const
{
	switch (inf) {
		...
		case info_flags::real:
            return basis.info(inf) && exponent.info(info_flags::integer);
        case info_flags::power_expanded:
            return (flags & status_flags::power_expanded);
		case info_flags::expanded:
			return (flags & status_flags::expanded);
		case info_flags::positive:
            if(basis.info(info_flags::real) && exponent.info(info_flags::even))
                return true;
            return basis.info(info_flags::positive) && exponent.info(info_flags::real);
        case info_flags::negative:
            if(basis.info(info_flags::real) && exponent.info(info_flags::even))
                return false;
            return (basis.info(info_flags::negative) && exponent.info(info_flags::real));
...
{
    const ex &mapped_basis = f.expr_visitor(basis);
    const ex &mapped_exponent = f.expr_visitor(exponent);
...
ex power::eval() const
{
//    std::cout<<"basis3 "<<basis<<std::endl;
    ...
    // ^(\infty, x)
    if (is_exactly_a<infinity>(basis)) {
        if (exponent.is_zero())
            throw(indeterminate_error("indeterminate expression: "
                                      "power::eval(): pow(Infinity, 0) is encountered."));
        ...
        if (basis.info(info_flags::positive)) {
            ...
            if (exponent.info(info_flags::positive))
                return pow(_ex_1,exponent)*Infinity;
        }

        throw(std::domain_error("power::eval(): pow(Infinity, c) is encountered."));
    }
	...
	if (exponent.is_zero()) {
		if (basis.is_zero())
            throw (indeterminate_error("indeterminate expression: "
                                      "power::eval(): pow(0,0) is encountered."));
        else if (basis.has(Infinity))
            throw (indeterminate_error("indeterminate expression: "
                                      "power::eval(): pow(Infinity,0) is encountered."));
		else
			return _ex1;
    ...
    //Turn (-n)^(1/2) into I*sqrt(n) where n is numeric (by me)
    if (basis.info(info_flags::negative) and (exponent.is_equal(_ex1_2) or exponent.is_equal(_ex_1_2))
        and !basis.is_equal(_ex_1))
        return dynallocate<mul>(pow(_ex_1,exponent),pow(-basis,exponent)).hold();
...
ex power::power_expand(unsigned options) const
{
    ...
    if (is_exactly_a<mul>(basis) && !basis.info(info_flags::indefinite)) {
        ...
        if (prodseq.size() > 0) {
            ...
            return (dynallocate<mul>(std::move(prodseq)) * pow(newbasis, exponent)).power_expand(options);
        } else
    ...
    if (are_ex_trivially_equal(basis,expanded_basis) && are_ex_trivially_equal(exponent,expanded_exponent))
        ...
        return dynallocate<power>(expanded_basis, expanded_exponent).setflag(options == 0 ? status_flags::power_expanded : 0);
...
//		for (auto & cit : m.seq) {
//            ex e=m.recombine_pair_to_ex(cit);
////            if ((is_a<power>(e) && e.op(1).info(info_flags::negative))//(2*3^(-1))^3->(2/3)^3,(2*exp(-1))^3->(2/exp(1))^3,
////                || (is_ex_the_function(e,exp) && e.op(0).info(info_flags::negative) )) {
////                return (*this).hold();
////            }
