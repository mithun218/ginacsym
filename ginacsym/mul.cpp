/** @file mul.cpp
 *
 *  Implementation of ginacsym's products of expressions. */
...
void mul::do_print_latex(const print_latex & c, unsigned level) const
{
	...
	if (!neg_powers.empty()) {
        ...
        const ex othereval=mul(others).eval();
        if ((!overall_coeff.is_equal(_ex1)) and//4*pow(3,_ex1_3)->4\times pow(3,_ex1_3), but x*pow(3,_ex1_3) is same form.
            (othereval).nops()>1 and is_a<power>(othereval.op(0)) and is_a<numeric>(othereval.op(0).op(0))
            and othereval.op(0).op(1)!=_ex1_2 ) {
            c.s<<"\\times";
        }
        ...
        if ((!overall_coeff.is_equal(_ex1)) and //4*pow(3,_ex1_3)->4\times pow(3,_ex1_3), but x*pow(3,_ex1_3) is same form.
            is_a<power>(*others.begin()) and is_a<numeric>((*others.begin()).op(0))
            and (*others.begin()).op(1)!=_ex1_2 ) {
            c.s<<"\\times";
        }
...
//    std::cout<<"basis1 "<<std::endl;
    ...
    if (std::any_of(seq.cbegin(), seq.cend(),
        ...
        s.reserve(seq.size());
        infinity result;
        bool is_inf=false,is_overall_coeff_mul_with_inf=false;
        for (const auto & elem : seq) {
            const numeric & num_coeff = ex_to<numeric>(elem.coeff);
            if ((!elem.rest.info(info_flags::positive) and !elem.rest.info(info_flags::negative))
                || (!elem.rest.info(info_flags::numeric) and !is_a<infinity>(elem.rest))){//y*oo=y*oo
                s.push_back(elem);
            }
            else{
                ex powval=dynallocate<power>(elem.rest,elem.coeff);
                if(!is_overall_coeff_mul_with_inf and is_a<infinity>(powval)){//3*oo=oo
                    result *= powval;
                    result *=overall_coeff;
                    is_overall_coeff_mul_with_inf=true;
                }
                else if(!(powval).is_zero()){//avoiding 0*oo and getting 5*oo=oo
                    result *= powval;
                    is_inf=true;
                }
                else{
                    s.emplace_back(powval,_ex1);
                }
            }
        }

        if(is_inf || is_overall_coeff_mul_with_inf)
            s.emplace_back((ex)result, _ex1);

        if (s.size()==1 && is_a<infinity>(s.begin()->rest)) {
            return s.begin()->rest;
        }

        return dynallocate<mul>(std::move(s), oc).hold();
    ...
    for (auto i = seq.begin(); i != seq.end(); i++) {
        ...
        if (unlikely(is_ex_the_function(i->rest, exp) and
            ...
            exp_count++;
            if (exp_count>1 or not num_coeff.is_equal(1) or
...
//    for (auto i = seq.begin(); i != seq.end(); i++) {
//        const numeric& num_coeff = ex_to<numeric>(i->coeff);
//        if (unlikely(is_ex_the_function(i->rest, exp) and num_coeff.is_integer())
//            or (is_exactly_a<power>(i->rest) and is_exactly_a<numeric>((i->rest).op(0)) and num_coeff.is_integer())) {
//            ++j;
//            for (auto k=j; k != seq.end(); ++k) {
//                const numeric& num_coeff1 = ex_to<numeric>(k->coeff);
//                if (unlikely(is_ex_the_function(k->rest, exp) and num_coeff1.is_integer())
//                    or (is_exactly_a<power>(k->rest) and is_exactly_a<numeric>(k->rest.op(0)) and num_coeff1.is_integer())) {
//                    if ((k->rest).op(1).is_equal((i->rest).op(1))) {
...
//        std::cout<<"p.c"<<p.coeff<<std::endl;
//        if(!p.coeff.info(info_flags::positive) and !p.coeff.info(info_flags::negative))
//            return res*p.coeff;
//        res *= p.coeff;
//        return res;
//    }
