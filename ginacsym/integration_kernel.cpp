 ...
 *
 *  Implementation of ginacsym's integration kernels for iterated integrals. */
...
numeric kronecker_symbol_prime(const numeric & a, const numeric & n)
{
	if ( n == 1 ) {
		...
		if ( ginacsym::smod(a,8) == 1 ) {
			return 1;
		}
		else if ( ginacsym::smod(a,8) == -1 ) {
			return 1;
		}
		else if  ( ginacsym::smod(a,8) == 3 ) {
			return -1;
		}
		else if ( ginacsym::smod(a,8) == -3 ) {
			return -1;
	...
	return ginacsym::smod( pow(a,(n-1)/2), n);
}
...
ex modular_form_kernel::series(const relational & r, int order, unsigned options) const
{
    ...
    ex res = do_subs_q_expansion.expr_visitor(P).series(qbar,order);
