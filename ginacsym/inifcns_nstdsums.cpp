...
ex convert_H_to_zeta(const lst& m)
{
    ...
    return filter2.expr_visitor(filter.expr_visitor(H(m, xtemp).hold())).subs(xtemp == 1);
}
    ...
    ex expr_visitor(const ex& e) override
	{
		...
		if (is_a<mul>(e)) {
			...
			if (Hlst.nops() > 0) {
                ...
                return recursion.expr_visitor(result);
			} else {
				return e;
			}
		...
		if (is_a<function>(e)) {
			...
			if (name == "H") {
				...
				if (parameter.op(0) == 0) {
                    ...
                    ex buffer = recursion.expr_visitor(H(newparameter, arg).hold());
					if (is_a<add>(buffer)) {
                    ...
                    res = recursion.expr_visitor(res).expand() / firstzero;
                    return unify.expr_visitor(res);
				}
			}
		}
		return e;
		...
		if (is_a<function>(e)) {
			...
			if (name == "H") {
				...
				if (parameter.op(0) == 0) {
					...
					if (allthesame) {
						map_trafo_H_mult unify;
                        return unify.expr_visitor((pow(H(lst{ex(-1)},1/arg).hold() - H(lst{ex(0)},1/arg).hold(), parameter.nops())
					...
					if (allthesame) {
						map_trafo_H_mult unify;
                        return unify.expr_visitor((pow(H(lst{ex(1)},1/arg).hold() + H(lst{ex(0)},1/arg).hold() + H_polesign, parameter.nops())
						       / factorial(parameter.nops())).expand());
					}
				}
		...
		if (is_a<function>(e)) {
			...
			if (name == "H") {
				...
				if (parameter.op(0) == 0) {
					...
					if (allthesame) {
						map_trafo_H_mult unify;
                        return unify.expr_visitor((pow(-H(lst{ex(1)},(1-arg)/(1+arg)).hold() - H(lst{ex(-1)},(1-arg)/(1+arg)).hold(), parameter.nops())
						       / factorial(parameter.nops())).expand());
					}
				} else if (parameter.op(0) == -1) {
					for (std::size_t i = 1; i < parameter.nops(); i++) {
					...
					if (allthesame) {
						map_trafo_H_mult unify;
                        return unify.expr_visitor((pow(log(2) - H(lst{ex(-1)},(1-arg)/(1+arg)).hold(), parameter.nops())
						       / factorial(parameter.nops())).expand());
					}
				} else {
					for (std::size_t i = 1; i < parameter.nops(); i++) {
					...
					if (allthesame) {
						map_trafo_H_mult unify;
                        return unify.expr_visitor((pow(-log(2) - H(lst{ex(0)},(1-arg)/(1+arg)).hold() + H(lst{ex(-1)},(1-arg)/(1+arg)).hold(), parameter.nops())
						       / factorial(parameter.nops())).expand());
					}
				}
...
static ex H_evalf(const ex& x1, const ex& x2)
{
	if (is_a<lst>(x1)) {
		...
		if (*(--morg.end()) == 0) {
			symbol xtemp("xtemp");
			map_trafo_H_reduce_trailing_zeros filter;
            return filter.expr_visitor(H(x1, xtemp).hold()).subs(xtemp==x2).evalf();
		}
		...
		if (cln::abs(x) >= 2.0) {
            ...
            res *= trafo.expr_visitor(H(m, xtemp).hold());
			if (cln::imagpart(x) <= 0) {
		...
		if (cln::abs(x-9.53) <= 9.47) {
			...
			if (has_minus_one) {
                ...
                res *= filter.expr_visitor(H(m, numeric(x)).hold()).evalf();
				return res;
			}
...
ex convert_H_to_Li(const ex& m, const ex& x)
{
	...
	if (is_a<lst>(m)) {
        return filter2.expr_visitor(filter.expr_visitor(H(m, x).hold()));
	} else {
        return filter2.expr_visitor(filter.expr_visitor(H(lst{m}, x).hold()));
	}
}
