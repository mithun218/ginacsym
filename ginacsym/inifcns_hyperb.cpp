...
static ex sinh_eval(const ex & x)
{
    ...
    if (is_a<infinity>(x)) {
        if (x.is_equal(Infinity))
            throw (std::runtime_error("sinh_eval(): sinh(Infinity) encountered"));
		return x;
	}
...
static bool sinh_info(const ex & x, unsigned inf)
{
    switch (inf) {
...
{
	return sinh(ginacsym::real_part(x))*cos(ginacsym::imag_part(x));
}

static ex sinh_imag_part(const ex & x)
{
	return cosh(ginacsym::real_part(x))*sin(ginacsym::imag_part(x));
}
...
REGISTER_FUNCTION(sinh, evalf_func(sinh_evalf).
                        eval_func(sinh_eval).
                        info_func(sinh_info).
...
static ex cosh_eval(const ex & x)
{
    ...
    if (is_a<infinity>(x)) {
        if (x.is_equal(Infinity))
            throw (std::runtime_error("cosh_eval(): cosh(Infinity) encountered"));
		return Infinity;
	}
...
{
	return cosh(ginacsym::real_part(x))*cos(ginacsym::imag_part(x));
}

static ex cosh_imag_part(const ex & x)
{
	return sinh(ginacsym::real_part(x))*sin(ginacsym::imag_part(x));
}
...
static ex tanh_eval(const ex & x)
{
    ...
    if (is_a<infinity>(x)) {
        ...
        if (x.is_equal(-Infinity))
			return _ex_1;
        // x is Infinity
        throw (std::runtime_error("tanh_eval(): tanh(Infinity) encountered"));
	}
...
static bool tanh_info(const ex & x, unsigned inf)
{
    switch (inf) {
    case info_flags::expanded:
    case info_flags::real:
...
REGISTER_FUNCTION(tanh, evalf_func(tanh_evalf).
                        eval_func(tanh_eval).
                        info_func(tanh_info).
...
static ex coth_eval(const ex & x)
{
    ...
    if (is_a<infinity>(x)) {
        ...
        if (x.is_equal(-Infinity))
			return _ex_1;
        // x is Infinity
        throw (std::runtime_error("coth_eval(): tanh(Infinity) encountered"));
	}
...
REGISTER_FUNCTION(coth, evalf_func(coth_evalf).
                        eval_func(coth_eval).
                        info_func(tanh_info).
...
static ex sech_eval(const ex & x)
{
    ...
    // sech(Infinity) -> error
    if (is_a<infinity>(x)) {
        if (x.is_equal(Infinity) or x.is_equal(-Infinity))
			return _ex0;
        // x is Infinity
        throw (std::runtime_error("sech_eval(): sech(Infinity) encountered"));
	}
...
REGISTER_FUNCTION(sech, evalf_func(sech_evalf).
                        eval_func(sech_eval).
                        info_func(sinh_info).
...
static ex csch_eval(const ex & x)
{
    ...
    if (is_a<infinity>(x)) {
        if (x.is_equal(Infinity) or x.is_equal(-Infinity))
			return _ex0;
        // x is Infinity
        throw (std::runtime_error("csch_eval(): csch(Infinity) encountered"));
	}
...
REGISTER_FUNCTION(csch, evalf_func(csch_evalf).
                        eval_func(csch_eval).
                        info_func(sinh_info).
...
REGISTER_FUNCTION(asinh, evalf_func(asinh_evalf).
                         eval_func(asinh_eval).
                         info_func(tanh_info).
...
REGISTER_FUNCTION(acosh, evalf_func(acosh_evalf).
                         eval_func(acosh_eval).
                         info_func(sinh_info).
...
static ex atanh_eval(const ex & x)
{
    ...
    if (is_a<infinity>(x)) {
        ...
        if (x.is_equal(-Infinity))
			return _ex1_2*Pi*I;
        // x is Infinity
        throw (std::runtime_error("arctanh_eval(): arctanh(Infinity) encountered"));
	}
...
REGISTER_FUNCTION(atanh, evalf_func(atanh_evalf).
                         eval_func(atanh_eval).
                         info_func(sinh_info).
...
{
    if (is_exactly_a<numeric>(x))
        return atanh(ex_to<numeric>(pow(x,_ex_1)));
...
static ex acoth_eval(const ex & x)
{
	...
	if (is_exactly_a<function>(x)) {
        ...
        if (is_a<infinity>(x)) {
                return _ex0;
                // x is Infinity
                //throw (std::runtime_error("arccoth_eval(): arccoth(Infinity) encountered"));
        }
...
REGISTER_FUNCTION(acoth, evalf_func(acoth_evalf).
                         eval_func(acoth_eval).
                         info_func(sinh_info).
...
{
    if (is_exactly_a<numeric>(x))
        return asinh(ex_to<numeric>(pow(x,_ex_1)));
...
REGISTER_FUNCTION(acsch, evalf_func(acsch_evalf).
                         eval_func(acsch_eval).
                         info_func(sinh_info).
...
{
    if (is_exactly_a<numeric>(x))
        return acosh(ex_to<numeric>(pow(x,_ex_1)));
...
static ex asech_eval(const ex & x)
{
        ...
        // asech(oo) -> Pi*I/2
        // asech(-oo) -> Pi*I/2
        // asech(Infinity) -> error
        if (is_a<infinity>(x)) {
                if (x.is_equal(Infinity) || x.is_equal(-Infinity))
			return Pi*I*numeric(1,2);
                // x is Infinity
                throw (std::runtime_error("arcsech_eval(): arcsech(Infinity) encountered"));
        }
...
REGISTER_FUNCTION(asech, evalf_func(asech_evalf).
                         eval_func(asech_eval).
                         info_func(sinh_info).
